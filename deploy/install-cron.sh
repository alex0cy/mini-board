...
add "17 3 * * * cd $ROOT/repo && docker run --rm --env-file $ROOT/env -v $ROOT/data:/data \$(grep '^MINIBOARD_IMAGE=' $ROOT/env | cut -d= -f2) python -m app.gh_traffic >> $ROOT/logs/traffic.log 2>&1 # mini-board"
